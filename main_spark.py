...
if __name__ == "__main__":
    ...
    s = time.time()
    X1 = spark.read.csv('X1.csv', header=True)
    from pyspark.sql.functions import UserDefinedFunction as udf, explode, collect_set
    ...
    fps = time.time()
    fpGrowth = FPGrowth(itemsCol="ner", minSupport=0.1)
    ...
    model.transform(new_df).groupBy("prediction").count().orderBy("count",ascending=False).show()
    print(f'FPGrowth time {time.time() - fps}')
    # def custom_f(row):
    #     out = row.ner
    #     for entity in out:
    #         entities[entity].add(row.id)
    # entities = new_df.rdd.flatMap(lambda x: x.ner.map(())).toDF(["id","dsa",'ner'])
    # print(entities)
    #     # # read the datasets
    #     # X1 = pd.read_csv("X1.csv")
    #     # X2 = pd.read_csv("X2.csv")
    #     # # # perform blocking
    #     # X1_candidate_pairs = block_with_attr_2(X1, attr="title")
    #     # X2_candidate_pairs = block_with_attr_2(X2, attr="name")
    #     # # save results
