...
def apply_ner_1(text, min_length=1, max_length=50, token_separators_pattern=None):
...
def block_X2(X: DataFrame, jeccard_lower_threshold, jeccard_tolerance, max_block_size, min_token_length,
    ...
    X['brand'] = X['brand'].parallel_apply(clean_brand)
    print(f'NER TIME : {time.time() - s}')
    X['NER_TEXT'] = X['NER'].parallel_apply(lambda x: ' '.join(x))
    X['TEXT_LEN'] = X['name'].parallel_apply(lambda x: len(x))
    ...
    sort_by_columns = ['NER_TEXT', 'NER_TEXT_ASC', 'NER_TEXT_REV', 'NER_TEXT_MID', 'price', 'brand', 'name', 'TEXT_LEN']
...
def block_X1(X: DataFrame, jeccard_lower_threshold, jeccard_tolerance, max_block_size,
    ...
    X['NLP'] = X['title'].parallel_apply(lambda x: apply_ner_1(x, min_token_length, max_token_length, pattern))
    print(f'NER TIME : {time.time() - s}')
    X['TEXT_LEN'] = X['title'].parallel_apply(lambda x: len(x))
    # X['NER_TEXT'] = X['NER'].parallel_apply(lambda x: ' '.join(x))
    # X['NER_TEXT_ASC'] = X['NER'].parallel_apply(lambda x: ' '.join(sorted(x)))
    # X['NER_TEXT_REV'] = X['NER'].parallel_apply(lambda x: ' '.join(sorted(x, reverse=True)))
    sort_by_columns = ['title', 'TEXT_LEN', 'NLP']
    return block_dataset_3(X, sort_by_columns, max_block_size, jeccard_tolerance, jeccard_lower_threshold)


def block_dataset_3(X, sort_by_columns, max_block_size, jeccard_tolerance, jeccard_threshold):
    s = time.time()
    candidate_pairs_real_ids = set()
    for column in sort_by_columns:
        X = X.sort_values(by=column)
        block = [(n[0], n[1]) for n in X[['id', 'NLP']].values]
        for i in range(len(block)):
            ...
            for j in range(i + 1, min(len(block), i + max_block_size)):
                ...
                if block_size > max_block_size or jeccard_passed > jeccard_tolerance:
                    break
                sen_j = block[j][1]
                similarity = sen_i.similarity(sen_j)
...
def run():
    ...
    X1_candidate_pairs = block_X1(X1, .95, 120, 24, 3, 90)
    X2_candidate_pairs = block_X2(X2, .37, 120, 130, 1, 10)
